...
Take one more look at `decideShipment`, because its signature is hiding the next piece. Its output is data: commands describing what should happen next. Its input is data too: the current `orders`. But where did that array's shape come from? A history of things that happened. Placed. Paid. Shipped. Cancelled. Every order in that list is the sum of its events, and right now that history is trapped in whatever your setters last did to your state. If the decisions leaving the core deserve to be values, so does the history entering it. The moment you write that down, state stops being a thing you mutate and becomes a thing you _compute_: a fold over everything that has happened. That's the next piece: **[State Is a Fold Over Events](/posts/state-is-a-fold-over-events)**. (And the pure core you just built, with its data-in, data-out tests? It's the raw material a later piece will feed to property-based testing.)
