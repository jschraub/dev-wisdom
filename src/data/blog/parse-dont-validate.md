...
At the end of [the last piece](/posts/errors-are-values) I made illegal states unrepresentable _inside_ the program. Then, in the last paragraph, I admitted the front door was still wide open. The `orders` our railway ran on came off the network as `any`, parsed from JSON some server _promised_ us, not the compiler. Every honest type we built downstream was standing on a value nobody ever actually checked.
